...
KEY_TO_FREQ = {
    ...
    # Top row suggestion:
    pygame.K_q: 1046.50,  # Q6
    pygame.K_w: 1174.66,  # W6
    pygame.K_e: 1318.51,  # E6
    pygame.K_r: 1400.50,  # R6
    pygame.K_t: 1446.37,  # T6
    pygame.K_y: 1538.41,  # Y6
    pygame.K_u: 1614.27,  # U6
    pygame.K_i: 1759.03,  # I6
    pygame.K_o: 1813.90,  # O6
    pygame.K_p: 1912.87   # P6
}
...
def draw_keyboard_hint(surface):
    ...
    middle_notes = ["C5", "D5", "E5", "F5", "G5", "A5", "B5", "K5", "L5"]
    ...
    # Top row labels (Q-I)
    top_keys = "QWERTYUIOP"
    top_notes = ["Q6", "W6", "E6", "R6", "T6", "Y6", "U6", "I7", "O6", "P6"]
    x_start3 = 60
    y_start_top = HEIGHT - 120 # Move it slightly above the other rows

    label3 = FONT.render("Top:", True, TEXT_COLOR)
    surface.blit(label3, (10, y_start_top + 8))

    for i, (key_char, note) in enumerate(zip(top_keys, top_notes)):
        x = x_start3 + i * (key_size + gap)
        pg_key = getattr(pygame, f"K_{key_char.lower()}")
        freq = KEY_TO_FREQ.get(pg_key)
        is_active = freq in active_frequencies if freq else False

        color = (186, 104, 200) if is_active else (40, 40, 55) # Purple for top row
        pygame.draw.rect(surface, color, (x, y_start_top, key_size, key_size), border_radius=4)
        
        char_text = FONT.render(key_char, True, (255, 255, 255) if is_active else TEXT_COLOR)
        surface.blit(char_text, (x + 10, y_start_top + 2))
